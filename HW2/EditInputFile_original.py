...
Z_SHIFT_M     = 0.0                 # shift receivers UP by 2000 m (ElAc: more negative)
...
def parse_source_xy(lines):
    ...
    if sx is None or sy is None:
        raise RuntimeError("Could not find 'source' line with x= and y= in the input file.")
    return sx, sy
...
def fix_all_rec_z(lines):
    ...
    out = []
    for ln in lines:
        if ln.strip().startswith("rec "):
            ln = re.sub(r"z=([-\d.]+)", "z=0.00", ln, count=1)
        out.append(ln)
    return out

def semicircle_points_ns(sx, sy, R, n, z_shift):
    """
    NS vertical plane: x fixed at sx, y varies, z describes a semicircle above ground.
    ElAc convention: z is negative upward.
    """
    phis = np.linspace(0.0, math.pi, n)  # 0..180 degrees
    xs, ys, zs = [], [], []
    for phi in phis:
        y = sy + R * math.cos(phi)     # horizontal offset in NS
        h = R * math.sin(phi)          # height above ground (>= 0)
        z = -(h + z_shift)             # shift everything up by z_shift
        xs.append(sx)
        ys.append(y)
        zs.append(z)
    return xs, ys, zs

def semicircle_points_ew(sx, sy, R, n, z_shift):
    """
    ...
    xs, ys, zs = [], [], []
    for phi in phis:
        x = sx + R * math.cos(phi)     # horizontal offset in EW
        h = R * math.sin(phi)          # height above ground (>= 0)
        z = -(h + z_shift)             # shift everything up by z_shift
        xs.append(x)
        ys.append(sy)
...
def main():
    ...
    air_lines = []
    ns_points = []  # for plotting: (y, z, R)
    ew_points = []  # for plotting: (x, z, R)

    for ring_idx, R in enumerate(RINGS_M, start=1):
        # NS semicircle → HX<ring>_<point>
        xs_ns, ys_ns, zs_ns = semicircle_points_ns(sx, sy, R, N_PER_SEMI, Z_SHIFT_M)
        for pt_idx, (x, y, z) in enumerate(zip(xs_ns, ys_ns, zs_ns), start=1):
            ...
            ns_points.append((y, z, R))

        # EW semicircle → HY<ring>_<point>
        xs_ew, ys_ew, zs_ew = semicircle_points_ew(sx, sy, R, N_PER_SEMI, Z_SHIFT_M)
        for pt_idx, (x, y, z) in enumerate(zip(xs_ew, ys_ew, zs_ew), start=1):
            name = f"HY{ring_idx}_{pt_idx}"
            air_lines.append(format_rec_line(x, y, z, name))
            ew_points.append((x, z, R))

    print(f"Generated {len(air_lines)} air-station receivers "
    ...
    if ns_points and ew_points:
        fig, (ax_ns, ax_ew) = plt.subplots(1, 2, figsize=(12, 5), constrained_layout=True)

        # NS plane: y vs z
        ys, zs, Rs = zip(*ns_points)
        ...
        plt.suptitle("Three-ring air stations (R=100, 300, 500 m) shifted up by 2000 m")
        plt.show()
